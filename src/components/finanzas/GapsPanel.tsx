...
const REASON_LABELS: Record<GapReason, string> = {
  ...
  'posible-bom-circular': 'Posible receta circular — rama cortada al bajar de nivel',
};
 ...
 */
export default function GapsPanel({ gaps, missingConsensoMonths, missingTcMonths }: Props) {
  const hasAnything = gaps.length > 0 || missingConsensoMonths.length > 0 || missingTcMonths.length > 0;
  if (!hasAnything) {
      ...
      {gaps.length > 0 && (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[420px] border-collapse text-sm">
            ...
            <tbody>
              {gaps.map((g) => (
                <tr key={g.productId} className="border-b border-slate-800/60 last:border-0">
