...
export default function TendenciaMensualTable({ rows, fuente }: Props) {
      ...
      <table className="w-full min-w-[520px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-800 text-left text-xs uppercase tracking-wide text-slate-500">
            ...
            <th className="py-2 pr-4 text-right font-medium">{fuente === 'pedidos' ? 'Pedidos' : 'Facturas'}</th>
            <th className="py-2 pr-4 text-right font-medium">{fuente === 'pedidos' ? 'Vendido' : 'Facturado'}</th>
            {showNC && <th className="py-2 text-right font-medium">Notas de crédito</th>}
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => {
            ...
            return (
              <tr key={r.month} className="border-b border-slate-800/60 last:border-0">
                ...
                {showNC && <td className="py-2 text-right text-amber-400">{formatCompactCurrency(r.notasCreditoMonto)}</td>}
              </tr>
            );
          })}
        </tbody>
